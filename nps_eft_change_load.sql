CREATE OR REPLACE PROCEDURE APDEV.NPS_EFT_CHANGE_LOAD IS
tmpVar NUMBER;
/******************************************************************************
...
******************************************************************************/
insert_count      NUMBER;
v_procname        VARCHAR2(30)                      := 'NPS_EFT_CHANGE_LOAD';
...
,to_number(replace(ltrim(trim(S.WD_PRIOR ),'$'),',','')) WD_PRIOR
,to_number(replace(ltrim(trim(S.WD_CHANGE ),'$'),',','')) WD_CHANGE
,to_number(replace(ltrim(trim(S.WD_REVISED ),'$'),',','')) WD_REVISED
,S.SC_STAR SC_STAR
,S.NUM_REMAINING NUM_REMAINING
...
'CHANGE'
from STAGING.STG_NPS_EFT_CHANGE s;

insert_count:=SQL%ROWCOUNT;

COMMIT;

RPTVIEWER.RPT_UTIL.SEND_MAIL(v_procname,RPTVIEWER.mail_pkg.array( 'Ora_LoadAlerts@NDGROUP.COM'),' INSERTED '||insert_count||' ROWS INTO APDEV.NPS_EFT_CHANGE TABLE ON ORACLE1');
