CREATE OR REPLACE PROCEDURE APDEV.IS_ACCDT_FRGV_ELIG
(acf_cur_arg         OUT      SYS_REFCURSOR,
...
-- claim exists for the loss_date and  policy_number passed in   
select count(*)
  into v_clm_cnt
...
select max(hp.eff_date), max(run_date) run_date
  into V_EFF_DT, V_VERS_RUN_DT
...
IF v_mrb_cnt > 0 and v_clm_cnt = 1  THEN  -- Check condition if both surchargeable offense exist and claim exist
