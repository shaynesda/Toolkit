CREATE OR REPLACE PROCEDURE STAGING.APDEV_COUNT_PRE_LOAD
IS
...
IF (polcount+namcount_diff+maa_driver_diff+pol_drv_cnt+missing_items_cnt+missing_name_cnt) <> 0
 ...
 THEN
     ...
     APDEV.APDEV_UTIL.SEND_MAIL('LOAD ALERT - STAGING TO APDEV '||pr_date,APDEV.MAIL_PKG.ARRAY('CVEERABOMMA@ndgroup.com'),POLICY_LIST);
 ...
 ELSE
    ...
    POLICY_LIST:='STAGING to APDEV Record count checks:'||chr(13)||chr(13)||
     ...
     APDEV.APDEV_UTIL.SEND_MAIL('LOAD STATUS - STAGING TO APDEV '||pr_date,APDEV.MAIL_PKG.ARRAY('CVEERABOMMA@ndgroup.com'),POLICY_LIST);
    -- DBMS_OUTPUT.put_line('Difference counts staging to apdev');
