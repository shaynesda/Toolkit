CREATE OR REPLACE PROCEDURE STAGING.STAGING_COUNT_LOAD
IS
...
*** staging tables STAGING.WF_WANG.*             *** 
***                                              ***
****************************************************/
...
-- load the previous days record into the slt record type
 -- commented out due to NEW_APDEV load being moved prior to APDEV load  therefore no need to verify file to staging table counts as it already happened in NEW_APDEV load.
...
--  Commented out to support switch to NEW_APDEV load running prior to APDEV load. File to Staging Table recon already performed.
/*
 IF   --Check Files are same as the last load
   ...
   THEN
     ...
     APDEV.APDEV_UTIL.SEND_MAIL('LOAD ALERT - ACU TO APDEV STAGING '||pr_date,APDEV.MAIL_PKG.ARRAY('dbalerts@ndgroup.com','dgilmore@ndgroup.com'),POLICY_LIST);
   ...
   THEN
    ...
    ELSE
     POLICY_LIST:='No error conditions occurred:'||chr(13)||chr(13)||
                   ...
                   chr(13)||rpad('Name',13,' ')||rpad(namcount_ext,17,' ')||namcount||
                   chr(13)||rpad('Surcharge',13,' ')||rpad(surcount_ext,17,' ')||surcount;
     APDEV.APDEV_UTIL.SEND_MAIL('LOAD STATUS - ACU TO APDEV STAGING '||pr_date,APDEV.MAIL_PKG.ARRAY('dbalerts@ndgroup.com','dgilmore@ndgroup.com'),POLICY_LIST);
     -- Decode above will assign the correct process date (s/b the Friday date if Monday (2nd day of the week) else s/b -1)
